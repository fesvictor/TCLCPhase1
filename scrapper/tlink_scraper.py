...
def tlink_scrape(link, page_limit=50, search_list=[]):
    ...
    while (True):
        if (counter == 45):
            print("*****15 second delay initated to prevent forum block*****")
            time.sleep(15)
            counter = 0
        ...
        if page_no == 1:
            ...
            tlink, np_link = tlink_scrape_main(link, search_list)
            links_retrieved = links_retrieved + tlink
            if np_link == []:
                return links_retrieved
        else:
            print("Page = " + str(page_no) + " Counter = " + str(counter) + " " + np_link)
            tlink, np_link = tlink_scrape_main(np_link, search_list)
            links_retrieved = links_retrieved + tlink
            if np_link == []:
                return links_retrieved
    return links_retrieved