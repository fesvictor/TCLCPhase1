    ...
    for party in party_list: #search if any party name in the sentence
        for party_name in party.get_name():
            if party_name in word[0]: # if sentence contains words found in scale database
                ...
                if search_scale('Polarity', 1, word[0], 'english'): # search whether this word in database
                    i += 1
                    addScale(word[3], word[2], word[1], party.get_name()[0], 0, 'party')
                    break
                elif search_scale('Polarity', 2, word[0], 'english'):
                    i += 1
    ...
    for leader in leader_list: 
        for leader_name in leader.get_name():
            if leader_name in word[0]: 
                ...
                if search_scale('Polarity', 1, word[0], 'english'):
                    i += 1
...
i = 0
fb_list = process_fb_data('TCLCPhase1/' + param['facebook.files'])
...
i = 0 
tweet_list = process_tweet_data('TCLCPhase1/' + param['twitter.files']) 
yearTable = definexYear(['17'], party_list, leader_list) 
...
i = 0 
others_list = [] 
others_list += process_json_data('TCLCPhase1/' + param['json.files']) 
others_list += process_malaysia_kini_data('TCLCPhase1/' + param['malaysiakini.files']) 
others_list += process_lowyat_data('TCLCPhase1/' + param['lowyat.files']) 
